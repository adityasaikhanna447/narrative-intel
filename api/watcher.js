...
const MID  = /india|modi|rahul|kejriwal|mamata|yogi|amit|election|vote|poll|parliament|lok sabha|rajya sabha|cabinet|congress|bjp|aap|tmc|rally|statement|controversial|defect|alliance|coalition|ed raid|cbi|manifesto|eci|mcc/i;
...
module.exports = async function handler(req, res) {
  ...
  if (!env.geminiKey || !env.resendKey || !env.toEmail) {
    return res.status(500).json({ error: 'Missing: GEMINI_API_KEY, RESEND_API_KEY, ALERT_EMAIL' });
  }
  ...
  for (const topic of WATCH_TOPICS) {
    try {
      ...
      for (const story of stories) {
        ...
        const brief = await generateBrief(story, env, log);
        if (brief) {
          alerts.push({ story, brief, score, topic: topic.name });
        } else {
          log.push(`[${topic.name}] Brief generation failed — skipping`);
        }
      }
    } catch (e) {
...
// GEMINI BRIEF
// ─────────────────────────────────────────────────────
async function generateBrief(story, env, log) {
...
Return ONLY valid JSON — no markdown, no explanation, starts with { ends with }:

{
  ...
  "history": "Key background most people do not know — 3-4 sentences with specific dates",
  "data": "2-3 key statistics as narrative sentences — make numbers feel consequential",
  "perspective_govt": "Government position — 2 sentences",
  "perspective_critic": "Opposition or expert critique — 2 sentences",
  ...
  try {
    ...
    if (data.error) {
      log.push(`GEMINI ERROR: ${data.error.message}`);
      return null;
    }
    ...
    if (s === -1 || e === -1) {
      log.push(`GEMINI ERROR: Non-JSON response — ${raw.slice(0, 100)}`);
      return null;
    }
    ...
    log.push(`GEMINI EXCEPTION: ${e.message}`);
    return null;
  }
}
...
async function sendEmail({ story, brief, score, topic }, env) {
  ...
  const urgencyColor = isHigh ? '#e8392a' : '#f0a820';
  const urgencyBg    = isHigh ? '#fff0ee' : '#fffbee';
  const urgencyLabel = isHigh ? '🔴 HIGH URGENCY' : '🟡 MEDIUM';
...
<table width="620" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:8px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,0.08);">
  ...
  <tr><td style="background:#07080b;padding:20px 28px;">
    <table width="100%" cellpadding="0" cellspacing="0"><tr>
      <td>
        <span style="font-family:Arial,sans-serif;font-size:13px;font-weight:900;letter-spacing:3px;color:#ffffff;">NARRATIVE</span>
        <span style="font-family:Arial,sans-serif;font-size:13px;font-weight:900;letter-spacing:3px;color:#e8392a;">INTEL</span>
        <span style="font-family:monospace;font-size:9px;color:#555;margin-left:8px;">LIVE ALERT</span>
      </td>
      <td align="right">
  ...
  <tr><td style="background:${urgencyBg};border-left:4px solid ${urgencyColor};padding:10px 28px;">
    <table width="100%" cellpadding="0" cellspacing="0"><tr>
      <td><span style="font-family:monospace;font-size:9px;font-weight:700;letter-spacing:2px;color:${urgencyColor};">${urgencyLabel} · ${topic.toUpperCase()}</span></td>
  ...
  <tr><td style="padding:6px 28px 0;">
    <span style="font-family:monospace;font-size:9px;color:#888;">${(story.source || '').toUpperCase()} · <a href="${story.link || '#'}" style="color:#3b7ef6;">READ ORIGINAL →</a></span>
  </td></tr>
  ...
  <tr><td style="padding:12px 28px 16px;">
    ...
    <div style="margin-top:8px;font-size:13px;color:#666;font-style:italic;line-height:1.5;">${brief.angle || ''}</div>
  </td></tr>
  ...
  <tr><td style="padding:14px 28px 0;"><div style="height:1px;background:#eeeeee;"></div></td></tr>
  ...
  <tr><td style="padding:14px 28px 0;">
    <div style="font-family:monospace;font-size:9px;letter-spacing:2px;color:#888;margin-bottom:6px;">GOVT POSITION</div>
    <div style="font-size:12px;color:#3b7ef6;line-height:1.7;margin-bottom:10px;">${brief.perspective_govt || ''}</div>
    <div style="font-family:monospace;font-size:9px;letter-spacing:2px;color:#888;margin-bottom:6px;">CRITIC / EXPERT</div>
    <div style="font-size:12px;color:#e8392a;line-height:1.7;">${brief.perspective_critic || ''}</div>
  </td></tr>
  ...
  <tr><td style="padding:0 28px 16px;">
    ...
    <div style="font-size:11px;color:#3b7ef6;line-height:2.2;">${(brief.suggested_sources || []).map(s => `→ ${s}`).join('<br>')}</div>
  </td></tr>

  <tr><td style="background:#f8f8fc;padding:14px 28px;border-top:1px solid #eeeeee;">
    <table width="100%" cellpadding="0" cellspacing="0"><tr>
      <td><span style="font-family:monospace;font-size:9px;color:#aaaaaa;">NarrativeIntel · Automated Research Engine</span></td>
      <td align="right"><span style="font-family:monospace;font-size:9px;color:#aaaaaa;">Score ${score}/99 · ${topic}</span></td>
    </tr></table>
  </td></tr>
  ...
  const subject = `${isHigh ? '🔴' : '🟡'} [NarrativeIntel] ${brief.video_title || story.title.slice(0, 60)}`;
